...
/** Legacy socket basename; default connections use a per-user filesystem path. */
export const DEFAULT_SOCKET = "memeora-daemon.sock";

function defaultSocket(): string {
	return (
		process.env.MEMEORA_SOCKET ||
		(process.platform === "win32"
			? DEFAULT_SOCKET
			: path.join(os.homedir(), ".memeora", DEFAULT_SOCKET))
	);
}
...
export class Client {
	...
	private constructor(socket: net.Socket) {
		...
		socket.on("close", () =>
			this.failAll(new Error("daemon closed the connection")),
		);
	}
	 ...
	 */
	static async connect(socket?: string): Promise<Client> {
		const sock = net.connect(resolveSocket(socket ?? defaultSocket()));
	...
	async ingest(
		...
		if (r.type === "ingested")
			return { added: r.added, reinforced: r.reinforced };
	...
	async context(
		scope: string,
	): Promise<{ statics: MemoryDto[]; dynamics: MemoryDto[] }> {
		const r = await this.call({ op: "context", scope });
		if (r.type === "context")
			return { statics: r.statics, dynamics: r.dynamics };
		throw this.unexpected(r);
	}
	...
	async bundle(
		...
		if (r.type === "bundle")
			return { statics: r.statics, dynamics: r.dynamics, memories: r.memories };
		throw this.unexpected(r);
	}
	...
	private call(request: unknown): Promise<Response> {
		return new Promise<Response>((resolve, reject) => {
			...
			if (body.length > MAX_MESSAGE_BYTES) {
				reject(
					new Error(
						`message exceeds MAX_MESSAGE_BYTES (${body.length} > ${MAX_MESSAGE_BYTES})`,
					),
				);
				return;
			}
