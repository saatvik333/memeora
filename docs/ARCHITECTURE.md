...
6. **`cli`** + **`hook`** binaries (shipped surface: `doctor`/`add`/`ingest`/`recall`/`context`/`list`/`forget`/`scope`/`dashboard`/`models`; hook event handlers — the planned `install`/`serve` subcommands became `scripts/install.sh` and the `memeora-daemon` binary instead).
