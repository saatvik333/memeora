...
9. **Ecosystem:** freeze + version the **IPC protocol** (capability handshake); publish `memeora-client` (Rust) + `@memeora/client` (TS); **host-descriptor** format + **conformance kit**; "add your harness" docs. **DONE.** Protocol: `Response::Hello` now carries `capabilities` (serde-default, back-compat); `Client` exposes `server_version`/`capabilities`/`supports`; stability policy in `docs/PROTOCOL.md` (additive changes don't bump `PROTOCOL_VERSION`, negotiate features via capabilities). The hook is **data-driven**: `crates/hook` is now lib+bin, a `HostDescriptor` (TOML) encodes scope/transcript field paths + inject style + ack + invocation gating; the three first-party descriptors live in `adapters/_descriptors/*.toml` (embedded via `include_str!` so built-ins == shipped files), and `--descriptor <path>` loads a community host. Conformance kit (`crates/hook/tests/conformance.rs` + `fixtures/<host>/*.json`) replays real payloads → asserts scope/inject/ack. new hosts start by copying a first-party `adapters/_descriptors/*.toml` descriptor (daemon-free). TS SDK in `sdk/ts/` (`@memeora/client`, Node `net` + the same framing). "Add your harness" guide: `docs/ADAPTERS.md`.
...
## Verification
- **Engine unit tests:** add→recall; near-dup reinforces; scoping isolates user/project; **hybrid(RRF) beats vector-only & BM25-only**; reranker lifts NDCG; `<private>`/secret content never persists through the engine boundary. *(Planned, to land with their features: contradiction→old `is_latest=false` via the NLI `updates` path; episode decay.)*
...
**B. Engine scope — MVP DECIDED (avoid overpromising "supermemory-quality").** v1 ships: `add`, `recall`, **hybrid search**, **profiles**, **dedup/reinforce**, and **`extends` linking** of moderately-similar neighbors — all **never hard-delete**. **Deferred** (need the gated NER/NLI stack or a generative model, so NOT in v1 despite earlier plans): **conservative `updates`** (NLI-gated contradiction → flip `is_latest`/soft-archive), `derives`, and **automatic forgetting** (v1 = TTL/`expires_at` + manual archive only — the decay/observation columns now exist in the schema, but the engine does not yet run decay or consolidation). This keeps quality honest while the no-LLM path proves out via the benchmark harness.
