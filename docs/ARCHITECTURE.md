...
- **Long-lived Rust daemon + thin clients (essential).** supermemory cold-starts a 200KB JS bundle every turn — tolerable only because it just makes a network call. Our engine loads embedding/NER models, so `memeora-daemon` (blocking IPC server + optional `axum` dashboard) holds models + DB **once**; the `rmcp` MCP server, the `memeora-hook` binary, the CLI, and the OpenCode shim are all thin clients over **cross-platform IPC** (`interprocess`: Unix domain socket / Windows named pipe).
...
- **Incremental capture (deferred — marked in code, needs a hook state dir first):** per-session `lastCapturedUuid` tracker (`~/.memeora/trackers/{id}`) → only new turns parsed/stored; optional "signal mode" (only turns near keyword triggers). Today the hook re-reads the transcript tail each capture (last-N-turns cap, ~8 KB blob cap).
...
│   └── cli/     `memeora` (clap): doctor/add/ingest/recall/context/list/forget/scope/dashboard/models → daemon client
├── adapters/
│   ├── _descriptors/ first-party host descriptors (claude/codex/antigravity .toml, embedded into the hook)
│   ├── claude-code/  .claude-plugin/plugin.json + hooks/hooks.json + commands/*.md + skills/*/SKILL.md + .mcp.json
│   ├── codex/        .codex-plugin/plugin.json + hooks/hooks.json + config.toml snippet + .mcp.json
...
├── sdk/ts/      @memeora/client — TypeScript SDK over the IPC protocol
└── .github/workflows/  CI (fmt/clippy/test/deny + a bun `ts` job) + release (dist)
```

Model cache/manifest helpers live in `crates/core/src/models.rs`; weights are never bundled
into the repo or binary — they download on first run or ship as an offline bundle (Risk F).

Verified crates: `rmcp = "1"` (1.7.0; stdio transport + `#[tool]`/`#[tool_router]`), `fastembed` (MVP embedder/reranker), `rusqlite`(+bundled sqlite, **static** `sqlite-vec`)+`rusqlite_migration`, `tokio` (daemon's dashboard/broadcast runtime + the MCP server's stdio transport), `clap`, `interprocess` (local socket / Windows named pipe), `serde`. **Deferred behind feature** (ort-alignment): `gline-rs`/`gliclass-rs` (or all-`orp` family `gte-rs`+`gline-rs`+`gliclass-rs` as the alternative ML stack). TS shim only: `@opencode-ai/plugin`.

### Universal MCP tools (identical in all four)

| Tool | Purpose |
| ------ | --------- |
| `recall` | Hybrid search (dense + BM25 + graph + temporal, RRF-fused) → matching memories |
| `remember` | Save a memory (fact/preference/episode) in a scope |
| `context` | Inject the full profile (static + dynamic) for a scope |
| `list` | List the most recent memories in a scope |
| `forget` | Soft-delete a memory by id — never hard-deleted, history preserved |
...
- **Context injection format (shipped, simpler than earlier plans):** a "use it naturally, don't force it" instruction line followed by a flat `- [kind] content` bullet per stable-then-recent memory (`format_context` in `crates/hook/src/lib.rs`; the MCP `context` tool instead splits `## Stable`/`## Recent` sections). No relative-time or similarity-% annotations yet.
- **Keyword nudge over silent auto-save:** inject a synthetic message telling the agent to call `remember` with judgment (scope/kind).
...
- **Transcript→memory formatting (shipped, privacy-hardened variant):** `memeora-hook::session_capture` derives a compact summary of the *work* — files edited (edit-tool `file_path`) and commands run (`Bash` `command`, truncated ~100c, ≤20 each) — then the recent user/assistant **text** turns, with an overall ~8 KB cap. The extractor turns the derived `edited files …` / `ran command …` lines into durable **episodes**. **Tool *result* bodies are deliberately NOT captured**: raw command output / fetched pages / read file contents are attacker-influenceable, so storing them verbatim would let a hostile repo seed memory (a write→inject loop). The **adapter** produces this blob (full `sanitize` applied — strip `<private>` + redact, idempotent with the engine boundary) and the **daemon's local extractor** turns it into memories — the seam where our no-LLM engine replaces supermemory's server-side LLM `entityContext`. *Deferred (needs the observation/provenance layer that can rank low-trust evidence apart):* capturing truncated tool-result bodies; `thinking`/`system-reminder` stripping; incremental `lastCapturedUuid` turn tracking.
...
- **Core views:**
  ...
  - **Profile** — static + dynamic facts. *(API served — `/api/context` — but no UI view yet.)*
  - **Timeline / decay** — recency + forgetting visualization. *(Planned, not yet built.)*
  - **CRUD** — forget with "never hard-delete" honored (soft-archive); *pin/edit are planned*.
  - **Live mode** — SSE pushes new memories as the daemon extracts them → watch memory form in real time (supermemory-like delight).
- **Auth/exposure:** binds to `127.0.0.1` only (the daemon refuses a non-loopback `MEMEORA_DASHBOARD_ADDR`), no network exposure — it's a local tool.

## CI/CD & release

- **CI (GitHub Actions, on PR/push, `.github/workflows/ci.yml`):** a `rust` job — `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --all-features -D warnings`, `cargo test --workspace --all-features`, cached via `Swatinem/rust-cache`; a `deny` job (`EmbarkStudios/cargo-deny-action`, advisories **+ license checks** — also satisfies the "verify model/dep license redistribution" risk); and a single `ts` job (bun) that builds/tests `sdk/ts` (incl. the proto-parity test), type-checks + builds `dashboard/`, and type-checks `adapters/opencode`. (TypeScript tooling is **bun** throughout — never pnpm/npm.) No separate MSRV job — the pinned `rust-toolchain.toml` is the MSRV gate.
- **Release (tag-triggered): `dist` (formerly cargo-dist)** — builds portable native artifacts for Linux x86_64+aarch64, macOS x86_64+arm64, and Windows x86_64, then generates **installers** (`curl|sh`, PowerShell, Homebrew, npm wrapper) + a GitHub Release with checksums. The current tier uses dynamically-linked ONNX Runtime assets; fully-static musl remains a future tier. Optional `release-plz` covers workspace versioning + changelog + crates.io publish.
...
- **Contributor tooling:** copy a first-party `adapters/_descriptors/` descriptor as a starting point; a **conformance test kit** (replay real stdin fixtures → assert correct IPC calls) so third-party adapters self-verify; "add your harness in N steps" docs; adapters may live in-repo (`adapters/`) or in separate community repos against the stable contract.
...
1. **TEMPR-style 4-channel recall.** **Shipped — 4 of 4 (Phase 2):** dense + BM25 + graph + temporal, fused by the *same* rank-based **RRF** (`k=60`, equal-weight lists — importance comes from rank, not per-strategy weights; per-channel cap before fusing):
   - **Graph channel** — activate candidates over shared canonical entities plus direct relationships. The current SQLite score is a saturating shared-entity term `min(shared_entities, 5) / 5` plus the strongest directly linked edge's kind-weighted, idle-decayed strength. It is deliberately rank-oriented for RRF: a direct link outranks an entity-only neighbour without claiming semantic or causal edge types that are not stored.
   - **Temporal channel** — a **rule-based** parser recognizes a conservative set of UTC-day cues: ISO dates, `&lt;n&gt; units ago`, and coarse relative anchors such as `yesterday`, `last week`, and `last month`. It matches the resulting interval against each memory's occurrence interval. Hindsight falls back to a `flan-t5-small` seq2seq for the residue; **we deliberately skip that** (degrade gracefully → stays no-LLM).
...
7. **Candidate repair** *(inspired by Understand-Anything `schema.ts`)* — the current local-LLM safety layer trims content, drops empty candidates, and coerces an invalid kind to `Fact`; malformed JSON elements are skipped independently and an unusable response falls back to the heuristic extractor. Node/edge validation, dangling-edge repair, and a `GraphIssue` audit trail remain future work.
...
6. **`cli`** + **`hook`** binaries (shipped surface: `serve`/`doctor`/`add`/`ingest`/`recall`/`context`/`list`/`forget`/`consolidate`/`scope`/`dashboard`/`models`; `serve` runs the daemon, while guided installation lives in `scripts/install.sh`).
7. **Adapters** by leverage: Antigravity (plugin bundle) → Claude Code → Codex (reuse hook binary) → OpenCode (TS shim). **DONE — see `adapters/`.** `memeora-hook` now has a per-host parser/renderer (`--host claude|codex|antigravity`, events `session-start`/`pre-invocation`/`stop`/`pre-compact`); MCP tools accept an optional scope and default it to `project_tag(cwd)`; `memeora scope [path]` resolves that tag daemon-free for the OpenCode shim. All four bundles assume the binaries are on `PATH` + the daemon is running (first-class installer is step 10).
...
9. **Ecosystem:** freeze + version the **IPC protocol** (capability handshake); publish `memeora-client` (Rust) + `@memeora/client` (TS); **host-descriptor** format + **conformance kit**; "add your harness" docs. **DONE.** Protocol: `Response::Hello` now carries `capabilities` (serde-default, back-compat); `Client` exposes `server_version`/`capabilities`/`supports`; stability policy in `docs/PROTOCOL.md` (additive changes don't bump `PROTOCOL_VERSION`, negotiate features via capabilities). The hook is **data-driven**: `crates/hook` became a library (its binary now ships from the cli crate — step 10), a `HostDescriptor` (TOML) encodes scope/transcript field paths + inject style + ack + invocation gating; the three first-party descriptors live in `adapters/_descriptors/*.toml` (embedded via `include_str!` so built-ins == shipped files), and `--descriptor <path>` loads a community host. Conformance kit (`crates/hook/tests/conformance.rs` + `fixtures/<host>/*.json`) replays real payloads → asserts scope/inject/ack. new hosts start by copying a first-party `adapters/_descriptors/*.toml` descriptor (daemon-free). TS SDK in `sdk/ts/` (`@memeora/client`, Node `net` + the same framing). "Add your harness" guide: `docs/ADAPTERS.md`.
...
> **Step-11 status (2026-06-20, on `main`).** The engine-evolution **core is shipped** as
> gated increments (full `scripts/check.sh` green each): session-aware **capture** (derives
> files/commands → episodes); heuristic **entity canonicalization** (`crate::entity` +
> `entities`/`memory_entities`, migration #4); **consolidation** (`proof_count` corroboration
> on distinct near-duplicates); the **forgetting engine** (`crate::dynamics` — Ebbinghaus
> `decayed_strength` lazy-at-read + floor, Hebbian strength cap, Cepeda spaced-stability);
...
- `crates/daemon/src/{run.rs,server.rs,engine.rs,dashboard.rs}`, `crates/mcp/src/lib.rs`, `crates/hook/src/{lib.rs,descriptor.rs,run.rs}`, `crates/cli/src/{main.rs,bin/*.rs}`
...
- **Engine unit tests:** add→recall; near-dup reinforces; scoping isolates user/project; **hybrid(RRF) beats vector-only & BM25-only**; reranker lifts NDCG; `<private>`/secret content never persists through the engine boundary; cue-based supersession flips `is_latest` and preserves the version chain; Ebbinghaus decay floors and Cepeda spaced-stability (`crate::dynamics`). *(Planned, to land with its feature: NLI-driven contradiction supersession.)*
...
- **MCP:** run `memeora-daemon` + `memeora-mcp`; exercise `recall`/`remember`/`context`/`list`/`forget` via MCP inspector / `codex mcp`.
...
- **Ecosystem:** IPC protocol versioned + capability handshake; copying a first-party `adapters/_descriptors/` descriptor yields a working adapter; conformance kit validates a third-party host descriptor against real fixtures.

## Open questions & risks (audit targets)

**A. ML-in-Rust — ⛔ `ort` CONFLICT CONFIRMED (was the plan's biggest hidden bug).** `fastembed` (5.16.0 in-lock) pins the current `ort` rc (rc.12 today); `gline-rs` 1.0.1 + `gliclass-rs` pin `ort =2.0.0-rc.9`. `ort`/`ort-sys` link the native ONNX runtime, and **Cargo's `links` rule forbids two versions linking the same native lib → they cannot share one workspace.** Decision (must prove `cargo build --locked` clean before committing): **MVP = `fastembed`-only** (embeddings + rerank + sparse) with **Tier-0 heuristic extraction**; **Tier-1 NER/relations (`gline-rs`/`gliclass-rs`) is deferred behind a feature flag**, unlocked only when either (a) `fastembed`'s `ort` and the fbilhaut family converge, or (b) we adopt the **all-`orp` family** instead (swap embeddings to `gte-rs` so `gte-rs`+`gline-rs`+`gliclass-rs` all sit on one `ort` rc, dropping `fastembed`). NLI cross-encoder shares whichever `ort` we standardize on. Daemon RAM with models loaded (~0.5–1.5GB) → lazy-load per capability.
**B. Engine scope — MVP DECIDED (avoid overpromising "supermemory-quality"); since superseded by the step-11/VISION-gap program (status notes above).** v1 shipped: `add`, `recall`, **hybrid search**, **profiles**, **dedup/reinforce**, and **`extends` linking** of moderately-similar neighbors — all **never hard-delete**. Since then, **cue-based (not yet NLI-gated) `updates`/supersession**, the **Ebbinghaus/Hebbian/Cepeda forgetting engine** (decay applied lazily at read time), and the **evidence/freshness observation layer** have all shipped (see the step-11 and VISION-gap Phase 1–3 status notes above). **Still deferred:** `derives` edges and **NLI-driven** contradiction detection — both need the gated NER/NLI or local-LLM stack (Tier-2/Phase 6).
...
**G. Dashboard UI — DECIDED: Svelte 5 + Vite + Sigma.js.** Sigma.js (WebGL) chosen over SVG diagramming because the memory graph grows large. Function-first; the graph is the centerpiece. (Styling evolved past the original "deliberately minimal, no design system" call: the shipped UI is a small design-token system with light/dark theming over `bits-ui` primitives — see the Dashboard section.)
